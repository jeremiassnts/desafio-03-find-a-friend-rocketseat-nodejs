...
export class GetPetsUseCase {
    ...
    async execute({ city, state, age, ambient, energy, independency, size }: GetPetsUseCaseRequest): Promise<GetPetsUseCaseResponse> {
        if (!city || !state) {
            throw new GetPetsMustHaveCity()
        }

        const pets = await this.petsRepository.getMany({ city, state, age, ambient, energy, independency, size }, this.orgsRepository)
        return {
            pets
        }
    }
